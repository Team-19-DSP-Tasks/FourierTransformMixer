...
class Images:
    ...
    def resize_all_images(self, image_instances):
        """
...
class Image_Processing_App(Images):
        ...
        if not self.ui.selectMode.signalsBlocked():
            if  selected_item == "Magnitude and Phase":
              for key in self.image_fft_components.keys():
                  if self.image_fft_components[key] == "Magnitude" or self.image_fft_components[key] == "Phase":
                      pass
                  else:
                        self.image_fft_components[key] = "Magnitude"                      
            elif selected_item == "Real and Imaginary":
                for key in self.image_fft_components.keys():
                  if self.image_fft_components[key] == "Real" or self.image_fft_components[key] == "Imaginary":
                      pass
                  else:
                      self.image_fft_components[key] = "Real"      
        else:
            pass
            ...
            if hasattr(self, "dragging") and self.dragging:
                ...
                if (
                    ...
                    if selected_fft_component is not None:
                        ...
                        sliced_comp = np.zeros_like(selected_fft_component) if self.handle_combobox_change() == "Inner" else np.ones_like(selected_fft_component)

                        sliced_comp[y_indices[0] : y_indices[1], x_indices[0] : x_indices[1]] = 1 if self.handle_combobox_change() == "Inner" else 0
    ...
    def plot_transparent_rectangle(self, canvas_id, x_start, y_start, x_end, y_end, alpha=0.5):
        """
        ...
        existing_rectangles = [p for p in canvas.patches if isinstance(p, patches.Rectangle)]
    ...
    def inverse_fft(self):
        ...
        if mode == 2:
            mags = np.zeros_like(self.magnitude_sum)
            phases = np.zeros_like(self.phase_sum)
            ...
            for i, ((key, value), (_, value_2)) in enumerate(
                ...
                if type == "Magnitude":
                    mags += 2 * self.weights[i] * image_inst.typed_shifted_components["Magnitude"] * value_2
                        
                elif type == "Phase":
                    phases += 2 * self.weights[i] * image_inst.typed_shifted_components["Phase"] * value_2

            mags = np.fft.ifftshift(np.exp(mags))
            phases = np.fft.ifftshift(phases)
            reconstructed = (mags * np.exp(1j * phases))

            return np.clip(np.abs(np.fft.ifft2(reconstructed)), 0, 255)


        elif mode == 1:
            real = np.zeros_like(self.real_sum)
            imaginary = np.zeros_like(self.imaginary_sum)

            for i, ((_, value), (_, value_2)) in enumerate(
                zip(self.image_instances.items(), self.inner_outer_dict.items())
            ):
                image_inst = value
                if selected_text == "Full Image":
                ...
                for key in self.image_fft_components.keys():
                    ...
                    print(type)
                    if type == "Real":
                        real += self.weights[i] * image_inst.typed_shifted_components["Real"] * value_2
                    elif type == "Imaginary":
                        imaginary += (
                            self.weights[i] * image_inst.typed_shifted_components["Imaginary"] * value_2
                        )

            print("using real imag")
            real = np.fft.ifftshift(np.exp(real))
            imaginary = np.fft.ifftshift(np.exp(imaginary))
            reconstructed_2 = (real + imaginary * 1j)
            return np.clip(np.abs(np.fft.ifft2(reconstructed_2)), 0, 225)
